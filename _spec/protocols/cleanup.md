...
1.  **Ask:** Prompt user:
    1)  **Archive:** Move to `_spec/tracks/closed/<project_id>/`, update status to `[x]`.
    2)  **Delete:** Permanently remove folder and registry entry.
    3)  **Skip:** Leave in registry as `[x]`.
2.  **Execute:**
    - If Archive: Move folder, update registry link, commit `chore(_spec): Archive track '<track_description>' to closed/<project_id>`.
