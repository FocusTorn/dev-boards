...
**✅ CORRECT - UV Cache Directory Configuration**:

Set `UV_CACHE_DIR` environment variable to enable hardlinks when cache and venv are on the same drive:

**Workspace Settings** (`.vscode/settings.json` or `.cursor/settings.json`):
```json
{
    "UV_CACHE_DIR": "D:\\_dev\\.AppData"
}
```
...
$env:UV_CACHE_DIR = "D:\_dev\.AppData"
```

**System Environment Variable** (Windows):
```powershell
# Set permanently for user
[System.Environment]::SetEnvironmentVariable("UV_CACHE_DIR", "D:\_dev\.AppData", "User")
```
...
**❌ INCORRECT - Not setting UV_CACHE_DIR when cache and venv are on different drives**:

```toml
# Wrong: Cache on C:, venv on D:, but not setting UV_CACHE_DIR
# This causes hardlink warnings and falls back to copying files
[tool.uv]
link-mode = "copy"  # Suppresses warning but doesn't enable hardlinks
```

**✅ CORRECT - Setting UV_CACHE_DIR to same drive as venv**:

```toml
# Cache moved to D: via UV_CACHE_DIR, venv on D:
# Both on same filesystem, hardlinks work (default behavior)
[tool.uv]
