...
function Show-MenuSelection {
    ...
    if (Get-Command Show-Menu -ErrorAction SilentlyContinue) {
        ...
        Write-Host ""
        $selected = Show-Menu -MenuItems $Options
        return $selected
    } else {
        # Fallback to Read-Host if PSMenu not available
        Write-Host $Title
...
function Show-YesNoPrompt {
    ...
    # Format: "Prompt? (Y/n):" where the default option is capitalized
    # DefaultNo=true means default is No (n), so show (N/y)
    # DefaultNo=false means default is Yes (Y), so show (Y/n)
    $defaultChar = if ($DefaultNo) { "N" } else { "Y" }
    $otherChar = if ($DefaultNo) { "y" } else { "n" }
    $promptText = "$Prompt ($defaultChar/$otherChar)"
    # Store lowercase version for actual default value
    $actualDefault = if ($DefaultNo) { "n" } else { "y" }
    
    # Display prompt in cyan, input will be white (default)
    Write-Host -NoNewline "${promptText}: " -ForegroundColor Cyan
    $response = Read-Host
    
    # If empty input, use default and show it
    if ([string]::IsNullOrWhiteSpace($response)) {
        $response = $actualDefault
        # Move cursor back and show the default value (prompt in cyan, value in white)
    ...
    # Add blank line after response unless NoBlankLine is specified
    if (-not $NoBlankLine) {
        Write-Host ""
    }
...
function Show-TextInput {
    ...
    # PSMenu might not have text input, so we'll use Read-Host with better formatting
    do {
        if ($DefaultValue -and -not [string]::IsNullOrWhiteSpace($DefaultValue)) {
            ...
            $input = Read-Host
            # If empty input and we have a default, use it and show it
...
function Show-MultiSelect {
    ...
    if ($null -eq $DefaultSelected) {
        # Default to all selected
        $DefaultSelected = @($true) * $Options.Count
    } elseif ($DefaultSelected.Count -ne $Options.Count) {
        # Pad with false if not enough values
        $DefaultSelected = $DefaultSelected + @($false) * ($Options.Count - $DefaultSelected.Count)
    }
    
    # Build initial selection indices (all selected by default)
    $initialSelection = @()
    for ($i = 0; $i -lt $Options.Count; $i++) {
    ...
    # Try PSMenu (Sebazzz/PSMenu) - https://github.com/Sebazzz/PSMenu
    if (Get-Command Show-Menu -ErrorAction SilentlyContinue) {
        Write-Host $Title -ForegroundColor Cyan
        Write-Host ""
        try {
            $selected = Show-Menu -MenuItems $Options -MultiSelect -InitialSelection $initialSelection -ErrorAction SilentlyContinue 2>$null
    ...
    # Try PowerShell Interactive Menu (bibistroc/powershell-interactive-menu) - https://github.com/bibistroc/powershell-interactive-menu
    $hasInteractiveMenu = (Get-Command Get-InteractiveMenuUserSelection -ErrorAction SilentlyContinue) -and (Get-Command Get-InteractiveMultiMenuOption -ErrorAction SilentlyContinue)
    if ($hasInteractiveMenu) {
        try {
            # Build menu items with default selections
            $menuItems = @()
            for ($i = 0; $i -lt $Options.Count; $i++) {
                $params = @{
                    Item = $Options[$i]
            ...
            $selectedOptions = Get-InteractiveMenuUserSelection -Header $Title -Items $menuItems -ErrorAction SilentlyContinue 2>$null
            if ($selectedOptions -and $selectedOptions.Count -gt 0) {
                Write-Host ""
    ...
    # Try ps-menu (chrisseroka/ps-menu) - https://github.com/chrisseroka/ps-menu
    if (Get-Command Menu -ErrorAction SilentlyContinue) {
        Write-Host $Title -ForegroundColor Cyan
        Write-Host ""
        try {
            $selected = Menu -menuItems $Options -Multiselect -ErrorAction SilentlyContinue 2>$null
            if ($selected -and $selected.Count -gt 0) {
                Write-Host ""
                return $selected
            }
    ...
    # Fallback: Show numbered list with checkboxes (all selected by default)
    Write-Host $Title -ForegroundColor Cyan
    Write-Host ""
    
    for ($i = 0; $i -lt $Options.Count; $i++) {
        $marker = if ($DefaultSelected[$i]) { "[X]" } else { "[ ]" }
        $num = $i + 1
        Write-Host "  $num. $marker $($Options[$i])" -ForegroundColor White
    }
...
# Function to create a boxed header
...
function Show-Status {
    ...
    Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    Write-Host "[chart] GitHub SSH Status"
    Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    Write-Host ""
            ...
            if ($exitCode -eq 0) {
                ...
                if ($output -match "admin:public_key" -or $output -match "needs the.*scope") {
                    ...
                    if ($refreshExitCode -eq 0) {
                        ...
                        Write-Host ""
                        $completed = Show-YesNoPrompt -Prompt "Did the refresh complete successfully? (or did you complete browser steps?)" -DefaultNo $false
                        if ($completed) {
...
function Ensure-GitHubCli {
    ...
    if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
        ...
        Write-Host ""
        $installGh = Show-YesNoPrompt -Prompt "Install GitHub CLI now?" -DefaultNo $false
        if ($installGh) {
            Write-Host ""
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Write-Host ""
            $authGh = Show-YesNoPrompt -Prompt "Authenticate GitHub CLI now? (Requires browser steps)" -DefaultNo $false
            if ($authGh) {
                Write-Host ""
...
function Collect-AllInputs {
    ...
    Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    Write-Host "[wrench] GitHub CLI Setup"
    Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    Write-Host ""
    ...
    if ($existingKeys.Count -gt 0) {
        Write-Host "[clipboard] Existing SSH keys found:"
        Write-Host ""
        ...
        $selected = Show-MenuSelection -Options $menuOptions -Title "Select SSH Key" -Prompt "Choose a key or create new"
        
        if ($selected -eq "Create new key" -or $null -eq $selected) {
            Write-Host ""
            Write-Host "Note: 'github_' will be automatically prefixed to the key name" -ForegroundColor Cyan
            $keyNameInput = Show-TextInput -Prompt "Enter SSH key name" -Required $true
            $keyNameInput = $keyNameInput.Trim()
        ...
        Write-Host "No existing SSH keys found. Creating a new key..." -ForegroundColor Cyan
        Write-Host ""
        Write-Host "Note: 'github_' will be automatically prefixed to the key name" -ForegroundColor Cyan
        $keyNameInput = Show-TextInput -Prompt "Enter SSH key name" -Required $true
        $keyNameInput = $keyNameInput.Trim()
    ...
    # Check SSH config and get user preference
    $script:UPDATE_SSH_CONFIG = $false
    if (Test-Path $SSH_CONFIG) {
        $configContent = Get-Content $SSH_CONFIG -Raw
        if ($configContent -match "Host github\.com") {
            $expectedKeyPath = "~/.ssh/$($script:SSH_KEY_NAME)"
            ...
            if ($identityFileMatch.Success) {
                ...
                $selectedKeyPath = "$env:USERPROFILE\.ssh\$($script:SSH_KEY_NAME)"
                $selectedKeyPath = $selectedKeyPath -replace '/', '\'
                
                if (-not ($existingKeyPath -like "*$($script:SSH_KEY_NAME)" -or $existingKeyPath -eq $selectedKeyPath)) {
                    $script:UPDATE_SSH_CONFIG = Show-YesNoPrompt -Prompt "Update existing configuration ($SSH_CONFIG)?" -DefaultNo $true
                }
            } else {
                $script:UPDATE_SSH_CONFIG = Show-YesNoPrompt -Prompt "Update existing configuration ($SSH_CONFIG)?" -DefaultNo $true
            }
    ...
    # Check local repo and get user preference
    $script:RECREATE_REPO = $false
    if (Test-Path "$GIT_REPO_DIR\.git") {
        # Show prompt and path first, then get input on prompt line
        # Format: (N/y) means N is default (capitalized), (Y/n) means Y is default
        # Default is Yes (recreate), so show (Y/n)
        $defaultChar = "Y"
        $otherChar = "n"
        $promptText = "Remove existing repository and recreate? ($defaultChar/$otherChar): "
        # Actual default value is lowercase "y"
        $actualDefault = "y"
        Write-Host $promptText -ForegroundColor Cyan -NoNewline
        Write-Host ""
        Write-Host "  - $GIT_REPO_DIR"
        # Save current cursor position (after path)
        $savedTop = [Console]::CursorTop
        $savedLeft = [Console]::CursorLeft
        # Move cursor back up 2 lines and to end of prompt
        $promptLine = [Console]::CursorTop - 2
        [Console]::SetCursorPosition($promptText.Length, $promptLine)
        # Read a single character without echoing to prevent overwriting prompt
        $key = [Console]::ReadKey($true)
        if ($key.Key -eq [ConsoleKey]::Enter) {
            # Enter pressed - use actual default and show it
            $response = $actualDefault
            Write-Host $actualDefault
            ...
            # Invalid key, use actual default
            $response = $actualDefault
            Write-Host $actualDefault
        }
        # Move cursor back to where we were (after path)
        [Console]::SetCursorPosition($savedLeft, $savedTop)
        Write-Host ""
        if ($response -match "^[Yy]$") {
            $script:RECREATE_REPO = $true
        } else {
            $script:RECREATE_REPO = $false
        }
    }
    ...
    $script:REMOVE_REMOTES = $false
    Push-Location $GIT_REPO_DIR
    $remotes = git remote 2>$null
    if ($remotes) {
        # Get remote output first
        $remoteOutput = git remote -v 2>$null
        # Show prompt and remotes first, then get input on prompt line
        # Format: (N/y) means N is default (capitalized), (Y/n) means Y is default
        # Default is Yes (remove and set up new), so show (Y/n)
        $defaultChar = "Y"
        $otherChar = "n"
        $promptText = "Remove existing remotes and set up new one? ($defaultChar/$otherChar): "
        # Actual default value is lowercase "y"
        $actualDefault = "y"
        Write-Host $promptText -ForegroundColor Cyan -NoNewline
        Write-Host ""
        # Show remotes list with 2 spaces before each line (matching repository path format)
        foreach ($line in $remoteOutput) {
            Write-Host "  - $line"
        }
        # Save current cursor position (after remotes)
        $savedTop = [Console]::CursorTop
        $savedLeft = [Console]::CursorLeft
        # Move cursor back up to prompt line (number of remotes + 1 for blank line)
        $linesToMove = $remoteOutput.Count + 1
        $promptLine = [Console]::CursorTop - $linesToMove
        [Console]::SetCursorPosition($promptText.Length, $promptLine)
        # Read a single character without echoing to prevent overwriting prompt
        $key = [Console]::ReadKey($true)
        if ($key.Key -eq [ConsoleKey]::Enter) {
        ...
        # Move cursor back to where we were (after remotes)
        [Console]::SetCursorPosition($savedLeft, $savedTop)
        Write-Host ""
        if ($response -match "^[Yy]$") {
            $script:REMOVE_REMOTES = $true
        } else {
            $script:REMOVE_REMOTES = $false
        }
    }
    ...
    # Convert to string and trim, handling null case
    if ($null -ne $gitUserName -and $gitUserName -ne "") {
        $script:GITHUB_USER = [string]$gitUserName
        $script:GITHUB_USER = $script:GITHUB_USER.Trim()
    } else {
        $script:GITHUB_USER = ""
    }
    
    # Check if username is valid (not null, empty, or whitespace)
    if ([string]::IsNullOrWhiteSpace($script:GITHUB_USER)) {
        $script:GITHUB_USER = Show-TextInput -Prompt "Enter your GitHub username" -Required $true
    } else {
        # Username was detected - capture it explicitly before building prompt
        $detectedUsername = [string]$script:GITHUB_USER
        $detectedUsername = $detectedUsername.Trim()
        
        # Double-check it's still valid
        if (-not [string]::IsNullOrWhiteSpace($detectedUsername)) {
            # Build the full prompt string with the username - use string concatenation to be explicit
            $confirmPromptText = "Confirm detected GitHub username: " + $detectedUsername
            $isCorrect = Show-YesNoPrompt -Prompt $confirmPromptText -DefaultNo $false
            if (-not $isCorrect) {
                $script:GITHUB_USER = Show-TextInput -Prompt "Enter your GitHub username" -Required $true
            }
        } else {
            # Username became empty after trimming, prompt for input
            $script:GITHUB_USER = Show-TextInput -Prompt "Enter your GitHub username" -Required $true
        }
    }
    
    # Default repository name to the workspace folder name (last segment of path)
    $defaultRepoName = Split-Path -Path $GIT_REPO_DIR -Leaf
    $script:REPO_NAME = Show-TextInput -Prompt "Enter repository name" -DefaultValue $defaultRepoName -Required $true
    $script:REPO_NAME = $script:REPO_NAME.Trim()
    
    # Ask about repository visibility
    $script:REPO_PRIVATE = Show-YesNoPrompt -Prompt "Make repository private?" -DefaultNo $true
    
    # Ask about git-crypt setup
    Write-Host ""
    $script:SETUP_GIT_CRYPT = Show-YesNoPrompt -Prompt "Set up git-crypt for encrypted secrets?" -DefaultNo $false
...
function Setup-Remote {
    ...
    if (Test-RepoExists $githubUser $repoName) {
        ...
        if ($script:REMOVE_REMOTES -or -not $remotes) {
            ...
            $createRepo = Show-YesNoPrompt -Prompt "Create repository on GitHub?" -DefaultNo $false
        }
        if ($createRepo) {
...
function Remove-GitCrypt {
    ...
    if (-not $Force) {
        Write-Host ""
        Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
        Write-Host "[ok] git-crypt removal complete!"
        Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    }
...
function Show-RemovalSummary {
    ...
    Write-Host ""
    Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    Write-Host "[chart] Removal Summary"
    Write-Host "------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"
    Write-Host ""
...
function Remove-All {
    ...
    # Show multiselect menu (default all selected)
    Write-Host "Select items to remove (default: all selected):" -ForegroundColor Cyan
    Write-Host ""
    $selectedItems = Show-MultiSelect -Options $removableItems -Title "Remove GitHub SSH Setup"
