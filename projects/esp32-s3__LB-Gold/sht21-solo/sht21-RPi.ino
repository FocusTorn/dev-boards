 ...
 * and publish to MQTT broker on Raspberry Pi
 * 
 * Hardware:
 ...
 * - WiFi (built-in)
 * - Wire (built-in)
 * - MQTT_RPi_Client (for MQTT connection to Raspberry Pi)
...
const unsigned long MQTT_PUBLISH_INTERVAL = 5000;  // Publish to MQTT every 30 seconds
...
void setup() {
  ...
  // Uses default WiFi and MQTT settings from library
...
void loop() {
  ...
  if (current_time - last_mqtt_publish >= MQTT_PUBLISH_INTERVAL) {
    if (sensor_valid) {
      ...
      Serial.println("⚠ Skipping MQTT publish - sensor data invalid");
    }
    publishStatus();
...
// MQTT FUNCTIONS
// ============================================================================
// Note: WiFi and MQTT connection are handled by MQTT_RPi_Client library

void publishStatus() {
  // Status is automatically published by MQTT_RPi_Client library
