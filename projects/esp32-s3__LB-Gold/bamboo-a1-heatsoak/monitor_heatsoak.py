...
class BambooLabPrinter:
    ...
    def is_paused(self, timeout: float = 3.0) -> Optional[bool]:
        ...
        if self.cert_file and Path(self.cert_file).exists():
            client.tls_set(ca_certs=str(Path(self.cert_file).resolve()))
    ...
    def resume(self, check_status: bool = True) -> bool:
        ...
        try:
            ...
            if not self.connected:
                print("❌ Failed to connect to printer MQTT broker")
                client.loop_stop()
                client.disconnect()
                return False
            ...
            if result.rc == mqtt.MQTT_ERR_SUCCESS:
                ...
                print(f"❌ Failed to publish resume command, return code: {result.rc}")
                client.loop_stop()
                client.disconnect()
                return False
...
class HeatSoakMonitor:
    ...
    def __init__(self, printer_ip: str, printer_access_code: str, printer_device_id: str,
                 ...
                 force_ready: bool = False, skip_status_check: bool = False,
    ...
    def start(self):
        ...
        try:
            ...
            if self.printer.cert_file and Path(self.printer.cert_file).exists():
                self.printer_mqtt_client.tls_set(ca_certs=str(Path(self.printer.cert_file).resolve()))
