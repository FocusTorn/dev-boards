...
pub fn detect_stage_change(
    ...
    } else if line_lower.contains("generating function prototypes")
        || line_lower.contains("generating prototypes")
    {
    ...
    } else if line_lower.contains("esptool")
        && line_lower.contains("elf2image")
        && line_lower.contains(".ino.elf")
        && line_lower.contains(".ino.bin")
        && !line_lower.contains("bootloader")
    {
        if compile_state.stage == crate::commands::compile_state::CompileStage::Linking
            || compile_state.link_stage_start.is_some()
        {
...
pub fn parse_compilation_info(line: &str, compile_state: &mut CompileState) {
    ...
    if line.contains("xtensa-esp32s3-elf-g++") || line.contains("xtensa-esp32s3-elf-gcc") {
        ...
        if line.contains("-c") {
            ...
            if compile_state.stage.rank()
                < crate::commands::compile_state::CompileStage::Compiling.rank()
            {
