use crate::app::{Action, App, DispatchMode, Focus, Message, MonitorType, TaskState};
...
fn create_test_app() -> App {
    ...
    profiles_tab_bindings.items = vec![
        crate::config::BindingConfig {
            ...
            triggers: [("ctrl+n".to_string(), "profile_new".to_string())]
                .into_iter()
                .collect(),
        },
        crate::config::BindingConfig {
            ...
            triggers: [("ctrl+c".to_string(), "profile_clone".to_string())]
                .into_iter()
                .collect(),
        },
        crate::config::BindingConfig {
            ...
            triggers: [("ctrl+d".to_string(), "profile_delete".to_string())]
                .into_iter()
                .collect(),
        },
        crate::config::BindingConfig {
            ...
            triggers: [("ctrl+s".to_string(), "profile_save".to_string())]
                .into_iter()
                .collect(),
        },
    ];
    ...
    let main_tab_bar_config = crate::config::TabBarConfig {
        ...
        tab_bindings: [("profiles".to_string(), profiles_tab_bindings)]
            .into_iter()
            .collect(),
        ..Default::default()
    };
    ...
    let main_tab_bar = crate::widgets::components::tabbed_bar::tabbed_bar::TabbedBar {
        config: crate::widgets::components::tabbed_bar::tabbed_bar::TabbedBarConfig {
            id: "MainContentTabBar".to_string(),
            style: crate::widgets::components::tabbed_bar::tabbed_bar::TabStyle::Tab,
            color: None,
            colors: None,
        ...
        items: vec![
            crate::widgets::components::tabbed_bar::tabbed_bar::TabItem {
    ...
    let output_button_bar = crate::widgets::components::button_bar::button_bar::ButtonBar {
        config: crate::widgets::components::button_bar::button_bar::ButtonBarConfig {
            id: "OutputPanelStaticOptions".to_string(),
            style: crate::widgets::components::button_bar::button_bar::ButtonBarStyle::BoxStatic,
            colors: None,
            alignment: crate::config::Alignment::default(),
            tabs: vec![],
        },
        items: vec![
            crate::widgets::components::button_bar::button_bar::ButtonItem {
                id: "autoscroll".to_string(),
                name: "Auto".to_string(),
                active: true,
            },
        ],
    ...
    let profile_id_button_bar = crate::widgets::components::button_bar::button_bar::ButtonBar {
        config: crate::widgets::components::button_bar::button_bar::ButtonBarConfig {
            id: "ProfileIDButtonBar".to_string(),
            style: crate::widgets::components::button_bar::button_bar::ButtonBarStyle::BoxStatic,
            colors: None,
            alignment: crate::config::Alignment::default(),
            tabs: vec![],
    ...
    let mut app = App {
        ...
        toast_manager: crate::widgets::toast::ToastManager::new(
            crate::widgets::toast::ToastConfig::default(),
        ),
...
mod dashboard {
    ...
    fn test_initialization() {
        ...
        assert!(app.running);
        assert!(app.main_tab_bar.items[0].active);
    }
    ...
    fn test_system_update_handling() {
        let mut app = create_test_app();
        let msg = "Test Output";
        app.update(Message::SystemUpdate(ProgressUpdate::OutputLine(
            msg.to_string(),
        )));
...
mod profiles {
    ...
    fn setup_profiles_tab() -> App {
        let mut app = create_test_app();
        app.main_tab_bar.set_active("profiles");
        app.layout = app.calculate_layout(app.view_area);
        app
    }
    ...
    #[test]
    fn test_profile_id_button_bar_save_click() {
        let mut app = setup_profiles_tab();
        let settings_layout = app.layout.settings.unwrap();

        // Click on "Save" button in the Profile ID field
        let area = settings_layout.field_areas[0];
        
        let event = MouseEvent {
            kind: MouseEventKind::Down(MouseButton::Left),
            column: area.x + 2, // Inside "[ Save ]"
            row: area.bottom() - 1,
            modifiers: KeyModifiers::empty(),
        };
        ...
        assert!(app.profile_id_button_bar.items.iter().any(|i| i.id == "save"));
    }
}
...
mod modals {
    ...
    fn test_modal_blocks_tab_switching() {
        ...
        let initial_tab = app.main_tab_bar.get_active_id().unwrap();

        // Simulate ']' which normally triggers NextTab
        app.tab_bar_map
            .get_mut("MainContentTabBar")
            .unwrap()
            .navigation
            .right = vec![ "]".to_string()];
        app.update(Message::Key(press(
            KeyCode::Char(']'),
            KeyModifiers::empty(),
        )));

        // Tab should NOT have changed
        let current_tab = app.main_tab_bar.get_active_id().unwrap();
